...
| SITE_URL | Required trusted HTTPS origin used for redirects; no path, query, credentials or fragment. Missing or invalid values keep checkout closed. Update when the domain is connected. |
...
Use the actual deployed origin for `SITE_URL`. The examples point to the current private review site; they do not make it publicly accessible or ready for Stripe. There is no fallback to the original, superseded hostname. A trailing slash is normalized away so success and cancellation returns use the same origin. See the [September 15 readiness review](business/current/AI-SF-Tour-Launch-Readiness.md) for the remaining dependencies.
...
The September 15 30-test suite passed, covering configured payment-return origins, pricing and government charges, age, dates/DST, atomic capacity, schema migrations, signatures, confirmation identity, tax snapshots, out-of-order refunds, duplicate events, complete/incomplete Stripe recovery, safe offline behavior, and checkout-return selection validation. TypeScript and the production build pass. Local HTTP checks confirm the homepage loads, availability remains closed, and checkout rejects payments while setup is unresolved. Earlier HTTP checks also covered Monday, invalid dates, cross-origin requests, unverifiable receipts, and the MLB feed.

Browser checks verified A/B selection preserves date, two guests and the optional return; review shows the matching branch/time and $470 subtotal. Phone date changes and Monday rejection were checked, along with phone navigation and page overflow at 320, 375, 430, 768 and 1280 pixels. Earlier GIF checks at 390 pixels covered the initial pause controls. The September 14 update removed those controls; the checks under Animated photographs below describe the current behavior.
